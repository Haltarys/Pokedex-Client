...
export const fetchUser = (params: IUserLogin): AppThunk<IApiResponse<IUser>> => (dispatch) => (
  UserService.fetchUser(params).then((res) => {
    if (res.status === 'success') dispatch(userActions.setUser(res.data as IUser));
    return res;
  })
);

export const registerUser = (body: IRegisterUser): AppThunk<IApiResponse<IUser>> => (dispatch) => (
